...
--EX1
SELECT a.nome, f.nome FROM ator a, atorestrelafilme aef, filme f
	WHERE a.idator= aef.idator
	AND aef.codfilme = f.codfilme; 
...
--EX2
SELECT nome c, telefone FROM cliente c 
	INNER JOIN telefoneCliente tl
	on tl.codcliente = c.codcliente;
	
--EX3
SELECT a.nome, f.nome, genero FROM ator a
	INNER JOIN atorestrelafilme aef
	on a.idator = aef.idator
	INNER JOIN filme f 
	on aef.codfilme = f.codfilme
...
--EX4 
SELECT nome, genero FROM filme 
	WHERE genero = 'Ação' or genero = 'Aventura';

--EX5
SELECT nome, dtaNasc FROM Ator 
	WHERE dtaNasc BETWEEN '10/10/1970' AND '10/10/1990';
	
--6
SELECT genero, COUNT(genero) FROM filme 
	GROUP BY genero
	ORDER BY genero;
